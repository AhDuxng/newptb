...
const generateFinalImage = async (backgroundColor) => {
  ...
  firstImage.src = photosInStrip.value ? photosInStrip.value[-1] : '';
...
const startCamera = async () => {
  ...
  try {
    stream = await navigator.mediaDevices.getUserMedia({
      video: {
        // Bạn có thể thử các kích thước ideal khác nhau
        width: { ideal: 1280 },
        height: { ideal: 720 },
        facingMode: 'user'
      },
...
// <<-- HÀM CHỤP ẢNH ĐÃ SỬA ĐỂ CẮT ẢNH -->>
const captureFrame = () => {
  ...
  if (videoAspectRatio > targetAspectRatio) {
    // Video rộng hơn target, cắt theo chiều ngang
    sourceWidth = videoHeight * targetAspectRatio;
    sourceX = (videoWidth - sourceWidth) / 2;
  } else {
    // Video cao hơn target, cắt theo chiều dọc
          ...
          <div class="flex md:flex-col gap-4 justify-center">
            ...
            <div @click="selectFrame('single')" class="cursor-pointer group">
              <div
                ...
                :class="[{ 'border-sky-500 ring-2 ring-sky-300': activeFrameType === 'single' }, 'border-gray-200']"
              >
                <div class="w-24 h-32 bg-gray-300 rounded-sm mx-auto flex items-center justify-center overflow-hidden">
                   <img v-if="activeFrameType === 'single' && photosInStrip.length > 0" :src="photosInStrip[-1]" class="w-full h-full object-cover">
                </div>
              </div>
              <p class="text-center mt-2 text-sm font-medium" :class="[{ 'text-sky-600': activeFrameType === 'single' }, 'text-gray-600 group-hover:text-sky-500']">Ảnh đơn</p>
            </div>
            ...
            <div @click="selectFrame('strip')" class="cursor-pointer group">
              <div
                ...
                :class="[{ 'border-sky-500 ring-2 ring-sky-300': activeFrameType === 'strip' }, 'border-transparent']"
              >
              ...
              <p class="text-center mt-2 text-sm font-medium" :class="[{ 'text-sky-600': activeFrameType === 'strip' }, 'text-gray-600 group-hover:text-sky-500']">Dải 4 ảnh</p>
            </div>
          </div>
        </div>
          ...
          <div v-if="isCameraOn && !isPhotoTaken" class="mb-6">
             <div class="flex space-x-4 overflow-x-auto pb-3 -mx-2 px-2">
               <div v-for="filter in filters" :key="filter.class" @click="applyFilter(filter.class)" class="flex-shrink-0 cursor-pointer text-center group">
                 <div class="w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-200" :class="[{ 'border-sky-500 ring-2 ring-sky-300': activeFilter === filter.class }, 'border-transparent']">
                   <img :src="previewImage" :class="filter.class" class="w-full h-full object-cover" alt="Preview bộ lọc">
                 </div>
                 <p class="mt-1.5 text-xs font-semibold transition-colors duration-200" :class="[{ 'text-sky-600': activeFilter === filter.class }, 'text-gray-600 group-hover:text-sky-500']">{{ filter.name }}</p>
               </div>
             </div>
           </div>
                ...
                <div class="flex items-center gap-3 bg-gray-200 p-2 rounded-full shadow-inner">
                  ...
                  <div class="flex items-center gap-2">
                    <span
                      ...
                      :class="[{ 'ring-2 ring-offset-2 ring-sky-500': frameColor === color }, 'ring-1 ring-gray-400']"
                    ></span>
                  </div>
